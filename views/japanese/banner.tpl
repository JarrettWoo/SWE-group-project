<div class="w3-container w3-center">
	<br>
	<br>
	<span class="w3-xxxlarge w3-margin"><b>自分のタースクブック</b></span>
	<span class="w3-display-topleft">
		<a class="w3-button w3-round w3-teal small-margin small-button" onclick="removeCompletedTasks()"> 
		終わったアイテムを消す </a>
	</span>
	<script> function removeCompletedTasks(){
		
		}
	</script>
	<span class="w3-display-topright">
		<a class="w3-button w3-round w3-teal small-margin small-button" href="/register">アカウントを作成</a>
		<a class="w3-button w3-round w3-teal small-margin small-button" href="/login">ログイン</a>
		<a class="w3-button w3-round w3-teal small-margin small-button" href="/logout">ログアウト</a>
		<button id="darkModeBtn" class="w3-button w3-round w3-gray small-margin small-button">ダークモード</button>
		<button id="calBtn" class="w3-button w3-round w3-gray small-margin small-button">カレンダー</button>
		<span class="w3-container">
			<div class="w3-dropdown-hover">
				<a class="w3-button w3-gray small-margin small-button">日本語</a>
				<div class="w3-dropdown-content w3-bar-block w3-border">
					<a href="/english" class="w3-bar-item w3-button">English</a>
					<a href="/japanese" class="w3-bar-item w3-button">日本語</a>
					<a href="/hindi" class="w3-bar-item w3-button">हिंदी</a>
				</div>
			</div>
		</span>
	</span>
</div>
